...
# Script to update backend URLs from localhost:5002 to localhost:5003

echo "Updating backend URLs from localhost:5002 to localhost:5003..."
echo "This will replace http://localhost:5002 with http://localhost:5003"
...
# Backup directory
BACKUP_DIR="port-update-backup-5003-$(date +%Y%m%d-%H%M%s)"
mkdir -p "$BACKUP_DIR"
...
for file in "${FILES_TO_UPDATE[@]}"; do
    if [[ -f "$file" ]]; then
        ...
        # Replace the URL
        sed -i '' 's|http://localhost:5002|http://localhost:5003|g' "$file"
